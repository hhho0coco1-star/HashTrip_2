...
설문 응답 수집
  → 질문별 선택 태그에 가중치 부여
  → TAG_WEIGHT_MASTER 기반 SQL 집계(SUM)로 합산
...
- **해결**: `TAG_WEIGHT_MASTER`(태그별 가중치)·`TRAVEL_RESULT_MAPPING`(결과 매핑) 테이블을 신설하고, `SUM(W.WEIGHT)` 기반 SQL 집계로 전면 수정 — 동일 태그 반복 선택 시 DB에서 합산 점수 누적
